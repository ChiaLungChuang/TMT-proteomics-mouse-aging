...
# Script:  mouseTMT_soleus.R
# Purpose: Per-tissue analysis for soleus — data reshaping,
...
# Input:   FD_Soleus.xlsx (TMT proteomics raw data)
# Output:  mouseTMT_So.csv, scatter plots as PDF
...
data <- read_excel("FD_Soleus.xlsx")
write.csv(data, "FD_Soleus.csv", row.names = FALSE)
data_TA <- read.csv("FD_Soleus.csv")
...
write.csv(data_TA, "data_mouse_Soleus.csv", row.names = FALSE)
...
data_TA_long <- data_TA %>%
  pivot_longer(
    ...
    names_pattern = "sig\\d+[NC]?..Sol_(\\d+M)(\\d+)_([SI])",
...
ratio_data_So_OY <- data_TA_wider %>%
  group_by(GN_ID) %>%
  summarize(
...
ratio_data_So_MY <- data_TA_wider %>%
  group_by(GN_ID) %>%
  summarize(
...
ratio_data_So_OM <- data_TA_wider %>%
  group_by(GN_ID) %>%
  summarize(
    Abundance_mean_middle = mean(Abundance[Age == "Middle"], na.rm = TRUE),
    Abundance_mean_old    = mean(Abundance[Age == "Old"],    na.rm = TRUE),
...
ratio_data_So <- ratio_data_So_OY %>%
  left_join(ratio_data_So_OM, by = "GN_ID") %>%
  left_join(ratio_data_So_MY, by = "GN_ID")

plot_data_So <- ratio_data_So %>%
...
write.csv(plot_data_So, "mouseTMT_So.csv", row.names = FALSE)
plot_data_So <- read.csv("mouseTMT_So.csv")
...
plot_data_So_filtered_OY <- plot_data_So %>%
  filter(ra.p.value_old_young <= 0.05)

top_genes_OY <- plot_data_So_filtered_OY %>%
  arrange(ra.p.value_old_young) %>%
  slice_head(n = 10)

plot_data_So_filtered_OY <- plot_data_So_filtered_OY %>%
...
plot_data_So_com_filtered_OY <- plot_data_So_filtered_OY %>%
...
mouseTMT_So_OY_top10 <- ggplot(plot_data_So_filtered_OY,
  ...
  labs(title   = "MouseTMT Soleus: Abundance FC vs. Ratio FC (Old/Young)",
...
print(mouseTMT_So_OY_top10)
ggsave("mouseTMT_So_OY_top10.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

# FC-threshold labels — Old vs. Young
mouseTMT_So_OY_label <- ggplot(plot_data_So_com_filtered_OY,
...
print(mouseTMT_So_OY_label)
ggsave("mouseTMT_So_OY_label.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

# Add R-squared annotation
mouseTMT_So_OY_label_R <- mouseTMT_So_OY_label +
  stat_poly_eq(aes(label = paste(after_stat(rr.label))),
               rr.digits = 3, parse = TRUE, color = "black", show.legend = FALSE)

print(mouseTMT_So_OY_label_R)
ggsave("mouseTMT_So_OY_label_R.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

# No labels — Old vs. Young
mouseTMT_So_OY_nolabel <- ggplot(plot_data_So_filtered_OY,
...
print(mouseTMT_So_OY_nolabel)
ggsave("mouseTMT_So_OY_nolabel.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

##### Middle vs. Young #####

plot_data_So_filtered_MY <- plot_data_So %>%
  filter(ra.p.value_middle_young <= 0.05)

top_genes_MY <- plot_data_So_filtered_MY %>%
  arrange(ra.p.value_middle_young) %>%
  slice_head(n = 10)

plot_data_So_filtered_MY <- plot_data_So_filtered_MY %>%
  mutate(Label_top = case_when(
    Gene_Name == "NA"              ~ NA_character_,
...
plot_data_So_com_filtered_MY <- plot_data_So_filtered_MY %>%
...
mouseTMT_So_MY_top10 <- ggplot(plot_data_So_filtered_MY,
  ...
  size_scale +
  scale_x_continuous(labels = scales::label_number(accuracy = 0.1)) +
  labs(title   = "MouseTMT Soleus: Abundance FC vs. Ratio FC (Middle/Young)",
       x       = "Log2 Abundance fold change (Middle/Young)",
       y       = "Log2 fold change in Insoluble/Soluble (Middle/Young)",
       caption = "Point size based on statistical significance of Age difference in insolubility") +
  tmT_theme

print(mouseTMT_So_MY_top10)
ggsave("mouseTMT_So_MY_top10.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

mouseTMT_So_MY_label <- ggplot(plot_data_So_com_filtered_MY,
                               aes(x = Abundance_FC, y = Ratio_FC, size = Size, label = Label)) +
  geom_point(alpha = 0.4, color = "maroon2") +
  geom_text_repel(vjust = 0.5, hjust = 0.5, size = 5, fontface = "bold",
                  family = "Arial", segment.color = "grey50", max.overlaps = Inf,
...
print(mouseTMT_So_MY_label)
ggsave("mouseTMT_So_MY_label.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

mouseTMT_So_MY_label_R <- mouseTMT_So_MY_label +
  stat_poly_eq(aes(label = paste(after_stat(rr.label))),
               rr.digits = 3, parse = TRUE, label.x = 1, label.y = 1,
               color = "black", show.legend = FALSE)

print(mouseTMT_So_MY_label_R)
ggsave("mouseTMT_So_MY_label_R.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

mouseTMT_So_MY_nolabel <- ggplot(plot_data_So_filtered_MY,
...
print(mouseTMT_So_MY_nolabel)
ggsave("mouseTMT_So_MY_nolabel.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

##### Old vs. Middle #####

plot_data_So_filtered_OM <- plot_data_So %>%
  filter(ra.p.value_old_middle <= 0.05)

top_genes_OM <- plot_data_So_filtered_OM %>%
  arrange(ra.p.value_old_middle) %>%
  slice_head(n = 10)

plot_data_So_filtered_OM <- plot_data_So_filtered_OM %>%
  mutate(Label_top = case_when(
    Gene_Name == "NA"              ~ NA_character_,
...
plot_data_So_com_filtered_OM <- plot_data_So_filtered_OM %>%
...
mouseTMT_So_OM_top10 <- ggplot(plot_data_So_filtered_OM,
  ...
  scale_size_continuous(range = c(1,9), name = "P-Value Size (-log10)",
                        ...
                        labels = c("1.30(p<0.05)","2","3","4","5")) +
  scale_x_continuous(labels = scales::label_number(accuracy = 0.1)) +
  labs(title   = "MouseTMT Soleus: Abundance FC vs. Ratio FC (Old/Middle)",
       x       = "Log2 Abundance fold change (Old/Middle)",
       y       = "Log2 fold change in Insoluble/Soluble (Old/Middle)",
       caption = "Point size based on statistical significance of Age difference in insolubility") +
  tmT_theme

print(mouseTMT_So_OM_top10)
ggsave("mouseTMT_So_OM_top10.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

mouseTMT_So_OM_label <- ggplot(plot_data_So_com_filtered_OM,
                               aes(x = Abundance_FC, y = Ratio_FC, size = Size, label = Label)) +
  geom_point(alpha = 0.4, color = "darkgreen") +
  geom_text_repel(vjust = 0.5, hjust = 0.5, size = 5, fontface = "bold",
                  family = "Arial", segment.color = "grey50", max.overlaps = Inf,
...
print(mouseTMT_So_OM_label)
ggsave("mouseTMT_So_OM_label.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

mouseTMT_So_OM_label_R <- mouseTMT_So_OM_label +
  stat_poly_eq(aes(label = paste(after_stat(rr.label))),
               rr.digits = 3, parse = TRUE, color = "black", show.legend = FALSE)

print(mouseTMT_So_OM_label_R)
ggsave("mouseTMT_So_OM_label_R.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

mouseTMT_So_OM_nolabel <- ggplot(plot_data_So_filtered_OM,
...
print(mouseTMT_So_OM_nolabel)
ggsave("mouseTMT_So_OM_nolabel.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

# ---- 6. Combined Three-Comparison Plots ----

combined_plot_data_mouseTMT_So <- bind_rows(
  plot_data_So_com_filtered_OM,
  plot_data_So_com_filtered_MY,
  plot_data_So_com_filtered_OY
)

write.csv(combined_plot_data_mouseTMT_So, "combined_plot_data_mouseTMT_So.csv", row.names = FALSE)
...
# Faceted — with labels
mouseTMT_So_combined_facet_label <- ggplot(combined_plot_data_mouseTMT_So,
  ...
  comparison_colors +
  scale_x_continuous(labels = scales::label_number(accuracy = 0.1)) +
  labs(title   = "MouseTMT Soleus: Abundance FC vs. Ratio FC (All Comparisons)",
...
print(mouseTMT_So_combined_facet_label)
ggsave("mouseTMT_So_combined_facet_label.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

# Faceted — no labels
mouseTMT_So_combined_facet_nolabel <- ggplot(combined_plot_data_mouseTMT_So,
...
print(mouseTMT_So_combined_facet_nolabel)
ggsave("mouseTMT_So_combined_facet_nolabel.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

# Overlapping (no facet) — no labels
mouseTMT_So_combined_nofacet_nolabel <- ggplot(combined_plot_data_mouseTMT_So,
                                               aes(x = Abundance_FC, y = Ratio_FC, size = Size, color = Comparison)) +
...
print(mouseTMT_So_combined_nofacet_nolabel)
ggsave("mouseTMT_So_combined_nofacet_nolabel.pdf", width = 7200, height = 6400, units = "px", dpi = 600)

# Overlapping (no facet) — with labels
mouseTMT_So_combined_nofacet_label <- ggplot(combined_plot_data_mouseTMT_So,
                                             aes(x = Abundance_FC, y = Ratio_FC, size = Size, label = Label, color = Comparison)) +
  geom_point(alpha = 0.3) +
  geom_text_repel(vjust = 0.5, hjust = 0.5, size = 5, fontface = "bold",
                  family = "Arial", segment.color = "grey50", max.overlaps = Inf,
...
print(mouseTMT_So_combined_nofacet_label)
ggsave("mouseTMT_So_combined_nofacet_label.pdf", width = 7200, height = 6400, units = "px", dpi = 600)